    ...
    class CustomRenderPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            using (new ProfilingSample(cmd, "Depth Peeling"))
            {
                ...
                {
                    depthRTs.Add(
                        Shader.PropertyToID($"_DepthPeelingDepth{i}")); //_DepthPeelingDepth0 _DepthPeelingDepth1
                    colorRTs.Add(
                        Shader.PropertyToID($"_DepthPeelingColor{i}")); //_DepthPeelingColor0 _DepthPeelingColor1
                    cmd.GetTemporaryRT(colorRTs[i], pixelWidth, pixelHeight, 0);
                    cmd.GetTemporaryRT(depthRTs[i], pixelWidth, pixelHeight, 32, FilterMode.Point,
                ...
                for (var i = DepthPeelingPass - 1; i >= 0; i--)
                {
                    ...
                    if (i == DepthPeelingPass - 1)
                        pass = 1;
                    cmd.Blit(colorRTs[i], renderingData.cameraData.renderer.cameraColorTarget,
                        DepthPeelingBlendMaterial, pass);
