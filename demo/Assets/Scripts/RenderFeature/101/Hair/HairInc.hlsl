...
float SpecularKajiyaHair(float ShiftMap, float ShiftTangent, float PowerValue, float PowerScale, float3 N, float3 T,
                         float3 H)
{
...
// float4 HairFragment (in v2f input )
{
    ...
    //================== Kajiya  高光============================================== //
    float4 FinalColor = 0;
    float3 KajiyaSpecular1 = _First_HairColor * 0.1 * SpecularKajiyaHair(
        HairShift.r, _First_ShiftTangent, _First_AnisotropicPowerValue * 100, _First_AnisotropicPowerScale, N, B, H);
    float3 KajiyaSpecular2 = _Second_HairColor * 0.1 * SpecularKajiyaHair(
        HairShift.r, _Second_ShiftTangent, _Second_AnisotropicPowerValue * 100, _Second_AnisotropicPowerScale, N, B, H);
    ...
    float3 MarschnerSpecular1 = _Marschner1Intensity * HairSpecularMarschner(
        BaseMap.rgb, L, V, MeshNormal, _BackLit, _Area, 1 - _Marschner1Smoothness, _Eccentric);
    float3 MarschnerSpecular2 = _Marschner2Intensity * HairSpecularMarschner(
        BaseMap.rgb, L, V, N, _BackLit, _Area, 1 - _Marschner2Smoothness, _Eccentric);
    ...
    float3 AdditionLight = AdditionalLights(BaseMap.rgb, N, V, _Marschner1Intensity, 0, _Marschner1Smoothness,
                                            input.positionWS, _Marschner2Smoothness, _Marschner2Intensity);
    ...
    UNITY_BRANCH if (_EnableBackLight)
        BackLight = _BakcLightColor * BackLighting(L, V, N, 1, _BackLightLerp, _BackLightPow, _BackLightIntensity,
                                                   mainLight.color.rgb);
