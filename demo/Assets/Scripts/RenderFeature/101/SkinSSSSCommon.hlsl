...
TEXTURE2D(_SkinCurvatureMap);
SAMPLER(sampler_SkinCurvatureMap);
...
float4 SkinSSSFragment(Varyings input) : SV_Target
{
    ...
    float3 Diffuse = directLighting * BaseMap.rgb * mainLight.shadowAttenuation * mainLight.color + indirectDiffuse +
        additionalDiffuse * BaseMap.rgb;
    // return Diffuse.xyzz;
