...
Shader "czw/Scene/ASE/Water"
{
	Properties
	{
		...
		_WorldUV_Scale("WorldUV_Scale", Range( 0 , 1)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 appendResult24 = (float2(WorldPosition.x , WorldPosition.z));
				float4 tex2DNode12 = tex2D( _FlowMap, ( appendResult24 * 0.1 * _WorldUV_Scale ) );
				float2 appendResult22 = (float2(tex2DNode12.r , tex2DNode12.g));
				float2 lerpResult27 = lerp( texCoord31 , ( (appendResult22*2.0 + -1.0) + ( texCoord31 + ( _Speed * _TimeParameters.x ) ) ) , _FlowIntensity);
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = tex2D( _BaseMap, lerpResult27 ).rgb;
				float Alpha = 1;
...
ASEEND*/
//CHKSM=DF75FEA0A6102E7195FAF76E007FA26F1BB66AF2